...
function getSongsByKey(records, key) {
    const validRecords = records.filter(record => Object.prototype.hasOwnProperty.call(record, "key"));
...
function artistCounts(records) {
    const validRecords = records.filter(record => Object.prototype.hasOwnProperty.call(record, "artist(s)_name"));
    const artistList = hoffy.myFlatten(validRecords.map(record => record["artist(s)_name"]));
    
    return artistList.reduce((counts, artist) => {
        if (Object.prototype.hasOwnProperty.call(counts, artist)){
            counts[artist] += 1;
            return counts;
        }
        else {
