...
app.post('/quiz', (req, res) => {
    ...
    for (let i = 0; i < userAnswerLower.length; i++) {
        if (correctAnswerLower.includes(userAnswerLower[i]) && !(Object.prototype.hasOwnProperty.call(answerDict, userAnswerLower[i]))) {
            correctCounter ++;
            const original = correctAnswer.find(a => a.toLowerCase() === userAnswerLower[i]);
